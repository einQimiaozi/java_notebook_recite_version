...
5.增量更新和原始快照(STAB)：用于解决并发GC下用户线程修改引用关系导致三色标记错误的问题(即错误的将黑色节点删除，漏删不管)，使用写屏障实现(一小段钩子代码)
  - 为什么增量更新和原始快照可以解决：因为黑色节点引用白色节点同时灰色节点删除了对该白色节点的引用同时发生时，才有可能误删，此时的白色节点其实被黑色节点引用，但由于黑色节点已经扫描完了，所以白色节点不可能再次被标黑了，那么只要记录黑色节点对白色节点的新增引用(增量更新)或记录灰色节点对白色节点的删除(stab)其中一个引用关系并重新扫描即可
  - 增量更新：用户线程修改引用关系后如果导致黑色节点指向白色节点，就记录下该引用，重新标记阶段扫描，本质上是将引用关系中的黑色节点标记成灰色，缺点是效率低
  - STAB：用户线程修改引用关系后如果导致一个灰色节点对白色节点的引用被删除，则记录下这个被删除的引用关系，重新标记阶段，以该引用关系中的灰色节点为根，重新扫描(再说直白一点就是按照扫描开始那一刻的引用关系链进行扫描)，缺点就是会产生浮动垃圾，因为假设灰色节点删除了白色节点的引用，但是没有新的黑色节点去引用白色节点，那么该白色节点在本轮扫描中不会被删除，造成漏删
  - cms用增量更新，g1用stab，因为增量更新需要重新扫描灰色节点，效率低，而stab不需要重新扫描任何节点，只是按照扫描开始那一刻的引用关系链进行扫描
...
2.标记复制：
  ...
  - 6.空间担保：当survive空间不够的时候，除了OOM外，还可以选择设置空间担保策略
    ...
    - 如果满足大于条件，则尝试Young GC，否则直接进行Old GC
...
G1：
  ...
  每个region是一块连续的虚拟内存，而region之间不连续，逻辑上依然有老年代和新生代的概念，但内存上不再划分，另外较大的对象会被放入大对象区域(对象的大小超过一个region的一半就算大对象)，大对象的回收参考老年代，region的大小范围在[1-32]mb之间
  
  - 执行流程
    - 初始标记：STW，查找GC Roots和其直接子节点
    - 根节点扫描：扫描survive区中存在的跨代引用
    - 并发标记
    - 重新标记：STW，修正并发标记阶段的引用关系变化，使用通过写屏障实现的原始快照算法
    - 筛选清除：根据region的回收价值排序进行回收(G1可以设置最长回收时间，这一步就是根据排序结果保证回收时间在设置的值之内)，STW，这一步也就是所谓的Mixed GC(回收全部年轻代和部分老年代)
  
  - 目标：实现高吞吐量，满足GC对暂停时间的要求
  - 适用场景：
    - 需要和用户线程并发执行
    - 需要对GC停顿时间做较好的预测
    - 不希望牺牲大量吞吐性能
...
Card-Table：数组，每个index里存了一个page(512k)，当这个page的数据发生变化，该index保存的值置为dirty(1)，初始化时为0，是一种“我指向谁”的结构

Rset：HashTable结构，key为当前region的起始地址，value为一个集合，集合内保存了这个region中的各个数据页在card-table中的index，可以根据Rset找到引用当前region的对象，是一种“谁指向我”的结构

Cset：记录了每次要被GC的region的集合

跨代引用问题：old->young和old->old的引用，如果存在跨代引用，则必须要每次扫描老年代才能确定年轻代对象的可达性分析结果

Cset和Rset本质上是使用时间换空间的思想，辅助GC的，每次只需要扫描Cset中的Rset就可以知道当前Region被谁引用，每次扫描的时候只需要选年轻代region的Rset作为GC Roots，这样就可以解决old->young和old->old的跨代引用问题，不需要每次都扫描整个老年代

## JVM调参

1.处理器问题
  - 串行：适用于小应用，小数据，响应时间要求高
  - 并行：适用于中大型应用，对吞吐量要求高，对响应时间要求低
  - 并发：适用于中大型行用，对响应时间要求高

2.配置：
  - 堆大小：一般涉及三个方面：操作系统数据模型(32bit or 64bit)，虚拟内存限制，物理内存限制
  - 典型设置：
    - Xmx3550m：jvm最大可用内存为3550m
    - Xms3550:jvm最小可用内存为3550m，可与xmx相同，避免每次GC后jvm重新分配内存
    - Xmn2g：年轻代大小为2g，整堆大小=年轻代+老年代+持久代，持久代固定为64m，所以增大年轻代会缩小老年代，sun退ijan设置为整个堆大小的3/8
