...
5.增量更新和原始快照(STAB)：用于解决并发GC下用户线程修改引用关系导致三色标记错误的问题，使用写屏障实现(一小段钩子代码)
  - 增量更新：用户线程修改引用关系后如果导致黑色节点指向白色节点，就记录下该引用，重新标记阶段扫描，本质上是将引用关系中的黑色节点标记成灰色
  - STAB：用户线程修改引用关系后如果导致一个灰色节点对白色节点的引用被删除，则记录下这个被删除的引用关系，重新标记阶段，以该引用关系中的白色节点为根，重新扫描(本质上就是把白色节点标记成灰色，这个理解起来的话就是：一个没扫描完引用关系链条的结构中的某一个链条断了，所以你不知道这个断掉的链条后面的链条是不是完整的，那么记录一下，然后重新看一下断掉的链条后面的链条就ok)
