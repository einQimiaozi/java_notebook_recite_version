...
为什么要有jmm：jmm主要为了解决多线程共享内存进行通信时的内存安全和数据不一致问题的
...
Card-Table：数组，每个index里存了一个page(512k)，当这个page的数据发生变化，该index保存的值置为dirty(1)，初始化时为0，是一种“我指向谁”的结构，老年代中会开辟一块空间专门放卡表，表示当前老年代都引用了哪些index的年轻代
...
跨代引用问题：年轻代如果被老年代引用，那么每次需要遍历整个老年代找出存活年轻代，效率太低，使用卡表记录跨代引用，每次扫描cset中的rset就可以知道当前region是否被老年代引用(通过index)，不需要扫描整个老年代
