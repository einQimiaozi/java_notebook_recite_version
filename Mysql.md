...
聚簇索引：聚簇索引是一种索引排列方式，每个索引指向的数据行根据索引的顺序排布，比如字典中拼音a开头的字后续是拼音b开头的字，聚簇索引一般是主键索引，如果表中没有主键则第一个非null的唯一索引作为聚簇索引，如果还没有，则mysql会创建一行隐藏的row-id作为聚集索引，注意是否是聚簇索引和是否唯一没有什么关系，聚簇索引的本质就是b+树叶子节点上保存的是行数据本身，聚集索引在磁盘的物理空间并不是连续的，而是通过将多个block链式相连形成顺序存储！！！

非聚簇索引：索引指向的数据行和和顺序无关，比如索引数据行a在内存地址为1000的地方，它的下一个索引数据行b有可能在1001的地方，也有可能在2002，3004等等任何地方，非聚簇索引的本质就是b+树叶子节点上保存的是行数据所在的地址，所以需要二次查询，非聚簇索引也可以是唯一索引！！！
...
索引下推：条件查询时对于数据的非主键不做回表，而是直接使用逻辑判断，不满足条件则跳过，举例 (a,20) 直接查询主键a的数据是否为20,不满足则跳过 (mysql5.6之前是先找主键为a的数据，然后再这堆数据里找id为20的数据)，所以索引下推触发的前提条件是你进行逻辑判断的数据刚好也是一个索引，一般都发生在联合索引时
...
不走索引的情况：
  ...
  - 7.关于select *:
    - 数据只有一行时直接走索引
    - 表中只有聚集索引：如果where后面包括创建聚集索引的时候的第一个字段，就会使用聚集索引
    - 表中只有非聚簇索引：如果where后面包括创建非聚集索引的时候的第一个字段，就会使用非聚簇索引
    - 表中两个都有：如果where后面包括创建聚集索引的时候的第一个字段，就会使用聚集索引，如果where后面包括创建非聚集索引的时候的第一个字段但不包括创建聚集索引的时候的第一个字段，就会使用索引查找，如果where后面不包括创建非聚集索引的时候的第一个字段和不包括创建聚集索引的时候的第一个字段，就会走全表扫描
...
3.不能使用的情况：
  ...
  - 常见情况：主键索引和唯一索引，因为这两种索引需要将数据读入内存判断是否有重复，所以不需要使用change buffer，而普通索引使用change buffer会效果拔群，比如，要插入 (4,400) 这个记录，就要先判断现在表中是否已经存在 k=4 的记录，而这必须要将数据页读入内存才能判断
...
1.redolog:记录每次提交的数据(在xx表做了xx修改，物理日志)，结构是一个数据，通过头指针和尾指针维护写入位置，数组满了就刷盘然后清空刷盘成功的数据(定时刷入，没到时间先放到缓冲区)，指针后移，用于实现crash-safe(前滚操作)，保证持久化，innodb中才存在的日志，写前日志(wal)，这也是为什么redo log可以做持久化，因为它是在数据写入前先写日志，所以即便数据库突然挂掉，redo log做的持久化只会比挂掉前数据库的内容更新，而不会更老

2.undolog：存放数据库修改前的数据，用于回滚操作，innode中才存在的日志，用于保证事务原子性和MVCC实现，内部就是一个由data_roll_ptr组成的链表日志，通过该链表可以回滚到不同的数据库版本，具体的说，一条数据如果被更新，那么旧版本(更新前的这条数据)会被写入undolog，然后新版本数据的roll_ptr会指向该旧版本
...
mvcc的实现原理：
  ...
  - 在读提交或可重复读的隔离级别下，事务在开始时会生成一个当前数据库的静态视图，该视图有以下属性
    ...
    - low_limit_id: 创建当前read view 时“当前系统最大事务版本号+1”，包含已提交和未提交的事务
    - up_limit_id: 创建当前read view 时“系统正处于活跃事务最小版本号”，包含已提交和未提交的事务
    - creator_trx_id: 创建当前read view的事务版本号
  - 也就是说，如果一个事务的data_trx_id小于low_limit_id，则必然可见
  - 如果一个事务的data_trx_id大于up_limit_id，则该事务是在当前静态图创建后才创建的，必然不可见
  - 如果一个事务在up和low之间：
...
锁队列和元数据锁：mysql中的读写锁也是读之间不互斥，但是元数据锁本质上是在一个锁队列中的，读锁a->读锁a之间是没问题的，但是读锁a->元数据锁a之间会互斥，此时如果读锁a由于延迟问题发生了阻塞，此时元数据锁a也会阻塞，如果后面再来一个新的读锁a，那么这个读锁a会因为队列优先级低(直白点说就是来的晚顺序靠后)，而被之前的元数据锁a阻塞！！！而元数据锁必须等待事务提交后才能释放，所以不论读写尽量不要使用长事务，因为一旦一个长事务执行时间过长，那么后面只要有一个互斥的添加字段操作被阻塞，则整个数据库就会陷入全库无法读写的状态，如果此时客户端有重试机制，每次阻塞都会再起一个会话重新请求的话......后果不堪设想
