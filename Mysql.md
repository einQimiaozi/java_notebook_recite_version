...
不走索引的情况：
  ...
  - 2.where条件查找有时会不走索引，原因：全表扫描本质上是顺序io，索引的话是随机io，在查找范围大的时候即便是有索引，随机io的效率可能还不如全表下的顺序io，所以mysql会自动选择效率较高的不走索引
  - 3.在where条件上进行函数or运算会不走索引，因为这样会破坏索引的有序性(底层是b+树嘛)，但是如果是先进行运算再判断条件就可以走索引，因为这样不会破坏有序性，比如select name from table where id+1=11(不走索引) select name from table where id=10+1(走索引)
  - 4.在联合索引下，不符合最左前缀原则就不会走索引了，比如索引(a,b,c)，其中a,b走索引,a,c走索引(但是这里走的是a的索引，c不走)，abc也走索引(这里顺序无所谓,只要有a在就行)，单查a也走索引，但是单查b or c，都不走索引，查询bc或cb也不走
  - 5.字符串->数字的隐式转换不走索引，但是数字->字符串的转换走索引
  - 6.使用!=会不走索引，is null不走索引，因为这几种情况能命中的范围比较大，所以mysql会认为全表扫描更快
  - 7.如果where查询中包含了索引和非索引的字段并且使用了or，那么所有索引都走不了
...
2.undolog：存放数据库修改前的数据，用于回滚操作，innode中才存在的日志，用于保证事务原子性和MVCC实现
...
mvcc的实现原理：
  ...
  - 注意区分：如果是读的话，默认是读undolog，因为mvcc下的默认隔离级别是可重复读，也就是读取的是事物开始时的静态视图，但是如果是写的话，默认是在当前读下进行数据更新！！！
