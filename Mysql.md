...
聚簇索引：聚簇索引是一种索引排列方式，每个索引指向的数据行根据索引的顺序排布，比如字典中拼音a开头的字后续是拼音b开头的字，聚簇索引一般是主键索引，如果表中没有主键则第一个非null的唯一索引作为聚簇索引，如果还没有，则mysql会创建一行隐藏的row-id作为聚集索引，注意是否是聚簇索引和是否唯一没有什么关系，聚簇索引的本质就是b+树叶子节点上保存的是行数据本身
